...
class CMSController extends BaseController
{
	...
	{
		return view('back-end/cms/home-page/new-home-page-section');
	}

	public function addHomePageSection()
	{
		//get logged-in user id
		$loggedInUserId = $this->session->get('user_id');

		// Load the HomePageModel
		$homePageModel = new HomePageModel();

		// Validation rules from the model
		$validationRules = $homePageModel->getValidationRules();
		...
		// If validation passes, create the home_page_section
		$data = [
            ...
            'content_blocks' => arrayToCommaString($this->request->getPost('content_blocks')),
		...
		// Call createHomePage method from the HomePageModel
		if ($homePageModel->createHomePage($data)) {
			//inserted user_id
			$insertedId = $homePageModel->getInsertID();
			...
			//log activity
			logActivity($loggedInUserId, ActivityTypes::HOME_PAGE_CREATION, 'Home page section created with id: ' . $insertedId);
			...
			//log activity
			logActivity($loggedInUserId, ActivityTypes::FAILED_HOME_PAGE_CREATION, 'Failed to create home page section with title: ' . $this->request->getPost('section_title'));
    ...
    public function policies()
    {
        ...
        // Set data to pass in view
        $data = [
            'policies' => $policiesModel->orderBy('created_at', 'DESC')->findAll(),
            'total_policies' => getTotalRecords($tableName)
        ];

        return view('back-end/cms/policies/index', $data);
    }

    public function newPolicy()
    {
        //no option for new policy
        return redirect()->to('/account/cms/policies');
    }
    ...
    public function addPolicy()
    {
        ...
        // Load the PoliciesModel
        $policiesModel = new PoliciesModel();

        // Validate the input
        $rules = $policiesModel->getValidationRules();
        if (!$this->validate($rules)) {
            return view('back-end/cms/policies/new-policy', ['validation' => $this->validator]);
        }

        // Prepare data for insertion
        $data = [
            'title' => $this->request->getPost('title'),
            'policy_for' => $this->request->getPost('policy_for'),
            'content' => $this->request->getPost('content'),
            'created_by' => $loggedInUserId,
            'updated_by' => $this->request->getPost('updated_by')
        ];
        ...
        if ($policiesModel->createPolicy($data)) {
            $insertedId = $policiesModel->getInsertID();
            ...
            //log activity
            logActivity($loggedInUserId, ActivityTypes::POLICY_CREATION, 'Policy created with id: ' . $insertedId);
            ...
            //log activity
            logActivity($loggedInUserId, ActivityTypes::FAILED_POLICY_CREATION, 'Failed to create policy with title: ' . $this->request->getPost('title'));

            return view('back-end/cms/policies/new-policy');
        }
    }
    ...
    public function editPolicy($policyId)
    {
        $tableName = 'policies';
        //Check if record exists
        if (!recordExists($tableName, "policy_id", $policyId)) {
            $errorMsg = config('CustomConfig')->notFoundMsg;
            session()->setFlashdata('errorAlert', $errorMsg);
            return redirect()->to('/account/cms/policies');
        }
        ...
        return view('back-end/cms/policies/edit-policy', $data);
    }
    ...
    public function updatePolicy()
    {
        ...
        if (!$this->validate($policiesModel->getValidationRules())) {
            return view('back-end/cms/policies/edit-policy', ['validation' => $this->validator, 'policy_data' => $policiesModel->find($policyId)]);
        }
        ...
        if ($policiesModel->updatePolicy($policyId, $data)) {
            session()->setFlashdata('successAlert', config('CustomConfig')->editSuccessMsg);
            logActivity($loggedInUserId, ActivityTypes::POLICY_UPDATE, 'Policy updated with id: ' . $policyId);
            return redirect()->to('/account/cms/policies');
        } else {
            session()->setFlashdata('errorAlert', config('CustomConfig')->errorMsg);
            logActivity($loggedInUserId, ActivityTypes::FAILED_POLICY_UPDATE, 'Failed to update policy with id: ' . $policyId);
            return redirect()->to('/account/cms/edit-policy/' . $policyId);
        }
    }

    public function viewPolicy($policyId)
    {
        $tableName = 'policies';
        //Check if record exists
        if (!recordExists($tableName, "policy_id", $policyId)) {
        ...
        return view('back-end/cms/policies/view-policy', $data);
    }
